...
class _GradesPageState extends State<GradesPage> with TickerProviderStateMixin {
  ...
  bool _teacherAllowsSubjectName(String subjectName) {
    ...
    if (_teacherSubjectsByClassYear.isNotEmpty) {
      ...
      final allowed = _teacherSubjectsByClassYear[_classYearKey(cls, year)] ??
          const <String>{};
  ...
  void initState() {
    ...
    DatabaseService.teacherAssignmentsVersion.addListener(
      _assignmentsListener,
    );
        ...
        if (_currentTeacherStaff != null) {
          if (_teacherSubjectsByClassYear.isNotEmpty) {
            ...
            subjects = subjects
                .where(
                  (c) =>
                      allowedSubjects.contains(c.name.trim().toLowerCase()),
                )
                .toList();
          }
        }
  ...
  Future<void> _applyAssignmentProfessors({
    ...
    final coursesForClass =
        await _dbService.getCoursesForClass(className, academicYear);
  ...
  Future<void> _snapshotPeriodDataOnLock({
    ...
    final courseIdByName = {
      for (final c in subjects) c.name: c.id,
    };
    ...
    for (final student in studentsInClass) {
      ...
      for (final subject in subjectNames) {
        ...
        final assigned = courseId.isNotEmpty
            ...
            : '';
        final currentProf =
            (row?['professeur'] as String?)?.trim() ?? '';
        final profToSave = assigned.trim().isNotEmpty
            ? assigned
            : currentProf;
  ...
  Future<void> _loadAllData() async {
    ...
    final courseNameById = {
      for (final c in allCourses) c.id: c.name,
    };
      ...
      if (_currentTeacherStaff != null) {
        ...
        if (assignments.isNotEmpty) {
          for (final a in assignments) {
            ...
            (_teacherSubjectsByClassYear[key] ??= <String>{})
                .add(subjectName.trim().toLowerCase());
          }
          classes = classes
              .where(
          ...
          classes =
              classes.where((c) => allowedClasses.contains(c.name)).toList();
        }
      } else {
        classes = [];
      }
          ...
          Row(
            ...
            children: [
              ...
              ElevatedButton.icon(
                ...
                label: const Text('Exporter classe PDF compact (ZIP)'),
                style: Theme.of(context).elevatedButtonTheme.style?.copyWith(
                  backgroundColor: MaterialStateProperty.all(
              ...
              ElevatedButton.icon(
                ...
                icon: const Icon(Icons.picture_as_pdf_outlined, size: 18),
                label: const Text('Exporter classe PDF ultra compact (ZIP)'),
                style: Theme.of(context).elevatedButtonTheme.style?.copyWith(
                  backgroundColor: MaterialStateProperty.all(
        ...
        return ValueListenableBuilder<String>(
          ...
          builder: (context, niveau, _) {
            ...
            Future<void> loadSubjectAppreciations() async {
              ...
              final coursesForClass =
                  await _dbService.getCoursesForClass(
                effClass,
                effectiveYear,
              );
              final courseIdByName = {
                                  ...
                                  children: [
                                    ...
                                    SizedBox(
                                      ...
                                      child: TextField(
                                          ...
                                          enabled: SafeModeService.instance
                                                  .isActionAllowed() &&
                                              !_isPeriodLocked(),
                                          decoration: InputDecoration(
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              if (info.director.trim().isNotEmpty) ...[
                                const SizedBox(height: 4),
                                Text(
                                  "${_adminCivility.isNotEmpty ? _adminCivility : 'M.'} ${info.director.trim()}",
                                  style: TextStyle(
                                    color: secondaryColor,
                                    fontWeight: FontWeight.bold,
                                    fontSize: 16,
                            ...
                            if (value == 'profile') {
                              ...
                              try {
                                ...
                                final className =
                                    selectedClass ?? student.className;
                                final academicYear =
                                    selectedAcademicYear ?? data['academicYear'];
                                await _applyAssignmentProfessors(
                                  className: className,
                                  academicYear: academicYear,
                                  subjectNames: subjectNames,
                                ...
                                if (editResult != null) {
                                  ...
                                  if (editResult.save) {
                                    ...
                                    for (final subject in subjectNames) {
                                      ...
                                      final app =
                                          (appreciations[subject] ?? '').trim();
                                      final moy =
                                          (moyennesClasse[subject] ?? '').trim();
                                      await _dbService
